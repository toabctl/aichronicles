...
// TestSearchEvents_GrepPatternFindable_ViaExtractions proves the
// end-to-end pipeline for Grep tool_use events: the GrepExtractor
// puts the pattern into the extractions table, and SearchEvents
// finds it via the third-tier extractions_fts fallback even when
// content_text doesn't mention the pattern.
func TestSearchEvents_GrepPatternFindable_ViaExtractions(t *testing.T) {
	t.Parallel()
	s := openTemp(t)
	now := time.Date(2026, 4, 24, 12, 0, 0, 0, time.UTC)
	...
	env := &ingest.Envelope{
		...
		EventID:         uuid.Must(uuid.NewV7()).String(),
		SourceAgent:     "claude-code",
		SourceSessionID: "sess-grep",
		Kind:            "tool_use",
		Role:            "tool",
		TsSource:        now.UTC(),
		Cwd:             "/work/grep",
		ContentText:     "ran a grep", // deliberately doesn't mention the pattern
		Tool:            &ingest.Tool{Name: "Grep"},
		Payload: map[string]any{
			"tool_input": map[string]any{
				"pattern": "uniquegrepterm",
			},
		},
		Transport: "hook",
		Redaction: &ingest.Redaction{Applied: true},
	...
	hits, err := SearchEvents(t.Context(), s.DB(), SearchEventOpts{Query: "uniquegrepterm"})
	if err != nil {
		t.Fatalf("SearchEvents: %v", err)
	}
	if len(hits) != 1 {
	...
	if !strings.HasPrefix(hits[0].Snippet.String, "[grep_pattern] ") {
		t.Errorf("expected [grep_pattern] snippet label, got %q", hits[0].Snippet.String)
	}
	if !contains(hits[0].Snippet.String, "uniquegrepterm") {
		t.Errorf("snippet should carry the pattern, got %q", hits[0].Snippet.String)
	}
}

// TestSearchEvents_WebSearchQueryFindable_ViaExtractions covers the
// web_query extraction path: querying a substring from a past
// search comes back tagged so the user can tell it surfaced
// because they searched for it (not because they wrote about it).
func TestSearchEvents_WebSearchQueryFindable_ViaExtractions(t *testing.T) {
	t.Parallel()
	s := openTemp(t)
	now := time.Date(2026, 4, 24, 12, 0, 0, 0, time.UTC)
	...
	env := &ingest.Envelope{
		...
		EventID:         uuid.Must(uuid.NewV7()).String(),
		SourceAgent:     "claude-code",
		SourceSessionID: "sess-search",
		Kind:            "tool_use",
		Role:            "tool",
		TsSource:        now.UTC(),
		Cwd:             "/work/search",
		ContentText:     "ran a web search",
		Tool:            &ingest.Tool{Name: "WebSearch"},
		Payload: map[string]any{
			"tool_input": map[string]any{
				"query": "uniquesearchphrase about pure-go ONNX",
			},
		},
		Transport: "hook",
		Redaction: &ingest.Redaction{Applied: true},
	...
	hits, err := SearchEvents(t.Context(), s.DB(), SearchEventOpts{Query: "uniquesearchphrase"})
	if err != nil {
		t.Fatalf("SearchEvents: %v", err)
	}
	if len(hits) != 1 {
	...
	if !strings.HasPrefix(hits[0].Snippet.String, "[web_query] ") {
		t.Errorf("expected [web_query] snippet label, got %q", hits[0].Snippet.String)
	}
}
