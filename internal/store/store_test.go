...
func TestOpen_FreshCreatesSchema(t *testing.T) {
	...
	if v != "9" {
		t.Errorf("schema_version: got %q, want 9", v)
	}

	// Expected tables all exist
	for _, name := range []string{"meta", "raw_envelopes", "sessions", "events", "events_fts", "events_fts_trigram", "extractions", "extractions_fts", "llm_outputs", "event_embeddings"} {
		var got string
