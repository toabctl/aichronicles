...
// defaultProbeTimeout caps the in-process UDS roundtrip the watchdog
// uses to prove the accept loop is alive. healthz is a literal
// {"status":"ok"} write — it has no DB call, no I/O, no allocation
// of consequence — so 500ms is comically generous. Picked that
// loose so a GC pause spike can't trigger a spurious restart.
...
//
// Concurrency: not safe for concurrent use; one Watchdog per daemon
// instance, owned by Start. Run blocks until ctx cancels.
type Watchdog struct {
	...
	// tests can observe pings without poking $NOTIFY_SOCKET.
	// In production it's sdDaemon.SdNotify.
...
// watchdog isn't enabled (manual aichroniclesd run, tests). The
// goroutine exits when ctx cancels, when the Notify callback
// errors fatally, or after a recovered panic.
//
// Returns nil in all "no watchdog" cases — the caller should treat
// the absence of a watchdog as configured-off, not an error.
...
// non-2xx, dial error) stay silent and let systemd's deadline trip.
//
// A panic inside the loop is recovered + logged so a bug in the
// ping path doesn't silently strand the goroutine — that would
// produce exactly the "process up, no pings" state we'd misread as
// the daemon's-stuck condition we built this to catch.
		...
		case <-tick.C:
			if w.probeOnce(ctx, client) {
				if _, err := w.Notify("WATCHDOG=1"); err != nil {
					// SdNotify only returns an error if the
					// payload is malformed or NOTIFY_SOCKET
					// can't be reached after we already
					// thought we were under systemd. Either
					// is unrecoverable for THIS goroutine —
					// log and exit so systemd's deadline
					// fires and we get a clean restart.
...
// probeOnce runs one in-process UDS roundtrip. Returns true iff the
// server answered 2xx within w.ProbeTimeout. Logs failures at warn
// level so a flapping daemon shows up in the journal even when the
// pings successfully resume on the next tick.
func (w *Watchdog) probeOnce(ctx context.Context, client *http.Client) bool {
	...
	if err != nil {
		// Dial timeouts, connection-refused, deadline-exceeded
		// from a hung accept loop all surface here. None of
		// these are panics; the deadline trips on its own.
...
// notify-type service. Wrapped here so cmd/aichroniclesd doesn't
// need to import the systemd lib directly.
...
// distinguish a clean shutdown from a watchdog-driven kill in the
// journal. Same no-op semantics as notifyReady.
...
// NotifyReady / NotifyStopping are public wrappers so cmd/aichroniclesd
// can call into them without re-importing sdDaemon. Trivially thin
// but they keep the systemd import-fan-out at one package.
