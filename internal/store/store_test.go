...
func TestOpen_FreshCreatesSchema(t *testing.T) {
	...
	if v != "5" {
		t.Errorf("schema_version: got %q, want 5", v)
	}

	// Expected tables all exist
	for _, name := range []string{"meta", "raw_envelopes", "sessions", "events", "events_fts", "events_fts_trigram", "extractions", "llm_outputs"} {
		var got string
