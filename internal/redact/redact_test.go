...
var positiveCases = []struct {
	...
	{"huggingface user token",
		"HF_TOKEN=hf_" + strings.Repeat("a", 37),
		"huggingface_token"},
	{"slack app-level token",
		"SLACK_APP_TOKEN=xapp-1-A0ABCDEF12-1234567890-" + strings.Repeat("a", 32),
		"slack_app_token"},
	{"sendgrid api key",
		"SENDGRID_API_KEY=SG." + strings.Repeat("a", 22) + "." + strings.Repeat("b", 43),
		"sendgrid_api_key"},
	{"github server token",
...
func TestDefault_AllPrefiltersAreCorrect(t *testing.T) {
	...
	cases := []struct {
		...
		{"gitlab_pat", "glpat-" + strings.Repeat("a", 20)},
		{"huggingface_token", "hf_" + strings.Repeat("a", 37)},
		{"slack_app_token", "xapp-1-A0ABCDEF12-1234567890-" + strings.Repeat("a", 32)},
		{"sendgrid_api_key", "SG." + strings.Repeat("a", 22) + "." + strings.Repeat("b", 43)},
