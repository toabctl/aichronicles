...
// activated us with, or (nil, nil) when socket activation is not
// in use. Callers should fall back to opening their own listener
// in that case.
//
...
//
// We currently support exactly one listening fd; the unit file
// ships one ListenStream= directive, so anything else is a
// misconfiguration we surface as an error rather than silently
// ignore.
...
// Serve runs the HTTP server on a caller-provided listener and
// returns a shutdown func that stops accepting new connections.
// Unlike ListenAndServe, it does not own the listener's backing
// socket — it will not chmod, unlink, or otherwise touch
// filesystem state. Use it when the listener came from systemd.
//
// The returned shutdown function takes a context so the caller
